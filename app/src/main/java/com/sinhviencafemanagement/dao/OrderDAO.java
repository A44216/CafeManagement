...
public class OrderDAO {
    ...
    private static ContentValues getOrderValues(Order order) {
        ...
        values.put(CreateDatabase.COLUMN_ORDER_ADDRESS_ID, order.getAddressId());
        return values; //
    }
    ...
    public List<Order> getOrdersByUserIdAndStatus(int userId, String status) {
        ...
        String selection = CreateDatabase.COLUMN_ORDER_USER_ID + " = ? AND " + CreateDatabase.COLUMN_ORDER_STATUS + " = ?"; //
        ...
        try (Cursor cursor = db.query( //
                ...
                selectionArgs, //
                null, null, //
                CreateDatabase.COLUMN_ORDER_DATE + " DESC" // Sắp xếp đơn mới nhất lên đầu
        )) {
            if (cursor.moveToFirst()) { //
                do {
            ...
            Log.e("OrderDAO", "Lỗi khi lấy đơn hàng theo status", e); //
        }
        return list; //
    }
