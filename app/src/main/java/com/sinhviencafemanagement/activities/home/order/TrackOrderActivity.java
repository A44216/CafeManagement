...
public class TrackOrderActivity extends AppCompatActivity {
    ...
    private void loadOrderData() {
        ...
        if (order != null) {
            ...
            btnCancelOrder.setEnabled(
                    CreateDatabase.ORDER_STATUS_PENDING.equals(order.getStatus())
            );
        }
    }
    ...
    private void cancelOrder() {
        Order order = orderDAO.getOrderById(orderId);
        if (order != null) {
            order.setStatus(CreateDatabase.ORDER_STATUS_CANCELED);
            int rows = orderDAO.updateOrder(order);
            if (rows > 0) {
                Toast.makeText(this, "Đã hủy đơn hàng!", Toast.LENGTH_SHORT).show();
                updateStepper(CreateDatabase.ORDER_STATUS_CANCELED);
    ...
    private void updateStepper(String status) {
        ...
        int colorCanceled = Color.parseColor("#F44336");
        ...
        if (CreateDatabase.ORDER_STATUS_COMPLETED.equals(status)) {
        ...
        } else if (CreateDatabase.ORDER_STATUS_CANCELED.equals(status)) {
            // Đơn đã bị hủy
            ...
            btnCancelOrder.setEnabled(false);
            btnCancelOrder.setBackgroundTintList(ColorStateList.valueOf(colorCanceled));
            btnCancelOrder.setText("Đã hủy đơn");
